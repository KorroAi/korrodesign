#!/usr/bin/env node
// KORRO STUDIO — Generate pipeline via OpenRouter (GLM 5.2 + Kimi K2.7)
...
if (!OPENROUTER_API_KEY) {
  console.error("[KORRO] OPENROUTER_API_KEY not set in environment.");
  console.error("[KORRO] Fallback: manual generation mode. The SKILL.md will guide Claude directly.");
  console.error("[KORRO] To enable full automation, add OPENROUTER_API_KEY to .env");
  process.exit(1);
}
...
async function generate() {
  console.log("[KORRO] Starting generation pipeline...\n");

  // ======================
  // STEP 1: GLM 5.2 — Architecture + Design
  // ======================
  console.log("[KORRO] Phase 1/3: GLM 5.2 designing architecture...");
...
- Concentric border radii throughout`;
  ...
  try {
    ...
    console.log("[KORRO] Architecture design complete.\n");
  } catch (e) {
    console.error(`[KORRO] GLM 5.2 failed: ${e.message}`);
    console.error("[KORRO] Falling back to manual design.");
    designSpec = "FALLBACK: Manual design required. Use the KORRO STUDIO RULES from SKILL.md.";
  }

  // Save design spec
  const specPath = join(outputDir, "KORRO_DESIGN_SPEC.md");
  writeFileSync(specPath, designSpec);
  console.log(`[KORRO] Design spec saved to ${specPath}\n`);

  // ======================
  // STEP 2: Kimi K2.7 — Code Generation
  // ======================
  console.log("[KORRO] Phase 2/3: Kimi K2.7 generating code...");
...
- Full implementation, NO placeholders`;
    ...
    try {
      ...
      console.log(`[KORRO] Generated ${component}.tsx`);
    } catch (e) {
      console.error(`[KORRO] Kimi failed for ${component}: ${e.message}`);
    }
  }
  ...
  // ======================
  console.log("\n[KORRO] Phase 3/3: Quality check & build...");
...
export const metadata: Metadata = {
  title: "KORRO Design Studio",
  description: "Generated by KORRO Design Studio v3",
};
...
`;
  ...
  // Install deps
  console.log("[KORRO] Installing dependencies...");
  try {
    execSync(`cd ${outputDir} && npm install`, { stdio: "inherit" });
  } catch {
    console.error("[KORRO] npm install had issues — may need manual fix.");
  }

  // Summary
  console.log(`\n[KORRO] Generation complete!`);
  console.log(`[KORRO] Files generated: ${files.length}`);
  files.forEach((f) => console.log(`  - ${f}`));
  console.log(`\n[KORRO] Start dev server: cd ${outputDir} && npm run dev`);
}

generate().catch((e) => {
  console.error(`[KORRO] Fatal error: ${e.message}`);
  process.exit(1);
});
